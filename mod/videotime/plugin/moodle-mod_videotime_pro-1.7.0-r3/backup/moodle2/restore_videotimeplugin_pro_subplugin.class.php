 ...
 * needed to restore one videotimeplugin_pro subplugin.
 */
class restore_videotimeplugin_pro_subplugin extends restore_subplugin {
    ...
    protected function define_videotime_subplugin_structure() {
        ...
        $elepath = $this->get_pathfor('/pro_settings');
    ...
    /**
     * Processes the videotimeplugin_pro element, if it is in the file.
     * @param array $data the data read from the XML file.
     */
    public function process_videotimeplugin_pro($data) {
        global $DB;

        $data = (object)$data;
        $data->videotime = $this->get_new_parentid('videotime');
        if (!empty($data->next_activity_id)) {
            $data->next_activity_id = $this->get_mappingid('course_module', $data->next_activity_id);
        }
        $DB->insert_record('videotimeplugin_pro', $data);
    }
}
